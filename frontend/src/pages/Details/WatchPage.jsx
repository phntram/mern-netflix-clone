...
import { ORIGIN_IMG_BASE_URL, SMALL_IMG_BASE_URL } from '../../utils/contants.js';
    ...
    if (loading) {
        return (
            <div className='min-h-screen bg-black p-10'>
                {/* The placeholder components that mimic the structure of the actual content while the data is being loaded. */}
                <WatchPageSkeleton />
            </div>
        );
    }
        ...
        <div className='bg-black min-h-screen text-white'>
            <div className='mx-auto container px-4 py-8 h-full'>
                ...
                <div className='flex flex-col md:flex-row items-center justify-between gap-20 max-w-6xl mx-auto'>
                    ...
                    <img src={ORIGIN_IMG_BASE_URL + content?.poster_path} alt="Poster image"
                        className='max-h-[600] rounded' />
                </div>
                ...
                {similarContent.length > 0 && (
                    <div className='mt-12 max-w-5xl mx-auto relative'>
                        ...
                        <div className='flex overflow-x-scroll scrollbar-hide gap-4 pb-4 group' ref={sliderRef}>
                            {similarContent.map((content) => {
                                ...
                                return (
                                    <Link key={content.id} to={`/watch/${content.id}`} className='w-52 flex-none'>
                                        <img src={SMALL_IMG_BASE_URL + content.poster_path} alt="Small Poster Image"
                            ...
                            <ChevronRight
                                className='absolute top-1/2 -translate-y-1/2 right-2 w-8 h-8 opacity-0  group-hover:opacity-100 transition-all duration-300 cursor-pointer bg-gray-600 text-white rounded-full '
                                onClick={scrollRight} />

                            <ChevronLeft
                                className='absolute top-1/2 -translate-y-1/2 left-2 w-8 h-8 opacity-0
								group-hover:opacity-100 transition-all duration-300 cursor-pointer bg-gray-600
